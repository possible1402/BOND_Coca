...
def soft_frequency(logits, power=2, probs=False):
    ...
    """
    if not probs:  
        softmax = torch.nn.Softmax(dim=1) # softmax(logits.view(-1, logits.shape[-1])):[batch size*max seq len,num labels]
        y = softmax(logits.view(-1, logits.shape[-1])).view(logits.shape) # logits:[batch size,max seq len, num labels]
    else:
        y = logits
    f = torch.sum(y, dim=(0, 1)) # f.shape:[num labels]
    t = y**power / f # t.shape:[batch size,max seq len,num labels]
    p = t/torch.sum(t, dim=2, keepdim=True) # torch.sum(t, dim=2, keepdim=True):[batch size,max seq len,1]

    return p # p:[batch size,max seq len,num labels]
...
def multi_source_label_refine(args, hp_labels, combined_labels, pred_labels, pad_token_label_id, pred_logits=None):
    if args.self_training_label_mode == "hard":
        ...
        if args.self_training_hp_label == 0: 
            ...
            _threshold = args.self_training_hp_label%1 # pred_labels:[batch size,max seq len,num labels];pred_labels.max(dim=-1)[0].shape:[batch size,max seq len]
            ...
            _threshold = args.self_training_hp_label%1
            _confidence = pred_labels.max(dim=-1)[0]  # _confidence:[batch size,max seq len]
            for i in range(0,pred_labels.shape[0]):  
                for j in range(1,pred_labels.shape[1]):  
                    if _confidence[i,j] < _threshold:
                        _distantlabel = combined_labels[i,j] # combined_labels:[batch size,max seq len]  
                        if _distantlabel.tolist()!=pad_token_label_id:
                            pred_labels[i,j] *= 0  
        ...
        return pred_labels, label_mask # label_mask:[batch size,max seq len],pred_labels[batch size,max seq len, num labels]


def multi_source_label_refine_for_each_category(args, label_confidence_map, combined_labels, pred_labels, pad_token_label_id, pred_logits=None):

    max_prediction=pred_labels.max(dim=-1)
    _confidence = max_prediction[0]  
    _pred_label_id= max_prediction[1] # _pred_label_id:[batch size,max seq len] 
    if args.self_training_hp_label_category==1: 
        for i in range(0,pred_labels.shape[0]):  
            for j in range(1,pred_labels.shape[1]):
                _distantlabel = combined_labels[i,j] # combined_labels:[batch size,max seq len] 
                if _distantlabel.tolist()!=pad_token_label_id:
                    _threshold = label_confidence_map[_pred_label_id[i,j].tolist()]  
                    if _confidence[i,j] < _threshold:
                        pred_labels[i,j] *= 0 
                        pred_labels[i,j,_distantlabel] = 1 
                label_mask =  (combined_labels!=pad_token_label_id) 
    if args.self_training_hp_label_category==0: 
        label_mask =  (combined_labels!=pad_token_label_id)
        for i in range(0,pred_labels.shape[0]): 
            for j in range(1,pred_labels.shape[1]):
                _distantlabel = combined_labels[i,j] # combined_labels:[batch size,max seq len] 
