    ...
    Examples::
        ...
        if labels is not None:
            ...
            if attention_mask is not None or label_mask is not None:
                ...
                # 测试代码 active_logits[:512]
            if labels.shape == logits.shape:
                loss_fct = KLDivLoss()
                if attention_mask is not None or label_mask is not None: # torch.nonzero(attention_mask[2,:]).size()[0]进行检验
                    active_labels = labels.view(-1, self.num_labels)[active_loss] # torch.nonzero(label_mask[2,:]).size()[0]进行检验
                    loss = loss_fct(active_logits, active_labels)
                else:
                    loss = loss_fct(logits, labels)
            else:
                loss_fct = CrossEntropyLoss() # 交叉熵损失函数计算时应该会自动忽略pad_token_label_id
                if attention_mask is not None or label_mask is not None:
            ...
            outputs = (loss,) + outputs
        return outputs  # (loss), logits,sequence_output,(hidden_states), (attentions)
