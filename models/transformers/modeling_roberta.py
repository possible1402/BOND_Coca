...
class RobertaForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        sequence_output = self.dropout(final_embeddings)
        logits = self.classifier(sequence_output)

        outputs = (logits,final_embeddings,) + outputs[2:]  # add hidden states and attention if they are here
        if labels is not None:
            # Only keep active parts of the loss
            if attention_mask is not None or label_mask is not None:
                active_loss = True
                if attention_mask is not None: # attention_mask:[batch size,max seq len]
                    active_loss = attention_mask.view(-1) == 1 # active_loss:[batch_size *max seq len]
                if label_mask is not None:
                    active_loss = active_loss & label_mask.view(-1)
                active_logits = logits.view(-1, self.num_labels)[active_loss] # active_logits:[不为0的数量,num labels]

            # 为什么在教师模型指导之后使用KL散度，而在指导之前使用交叉熵损失？
            if labels.shape == logits.shape: # labels:[batch size,max seq len,num labels]
                loss_fct = KLDivLoss()
                if attention_mask is not None or label_mask is not None:
                    active_labels = labels.view(-1, self.num_labels)[active_loss]# active_labels:[不为0的数量，num labels]
                    loss = loss_fct(active_logits, active_labels)
                else:
                    loss = loss_fct(logits, labels)
            else: # labels:[batch size,max seq len]
                loss_fct = CrossEntropyLoss()
                if attention_mask is not None or label_mask is not None:
